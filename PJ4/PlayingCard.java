...
class Deck {
        ...
        for (int s = 1; s <= 4 ; s++){
                for (int r = 1; r <= 13; r++){
                    try{
                        ...
                        System.out.println("Card Exc");
                    }
                }
            }
        playingDeck = new ArrayList<Card>(52);
        for(int i = 0; i < originalDeck.size(); i++){
            playingDeck.add(originalDeck.get(i));
        }
    ...
    public Deck(int n)
    {
        ...
        playingDeck = new ArrayList<Card>(numberDecks * 52);
        for(int i = 0; i < originalDeck.size(); i++){
            playingDeck.add(originalDeck.get(i));
        }
        // implement this method!
    ...
    {
        int cards = numberCards;
        List<Card> delt = new ArrayList<Card>(cards);
        List<Card> hand = new ArrayList<Card>(cards);
        try{
            if (numberCards > playingDeck.size()){
                throw new PlayingCardException("Not enough cards in playing deck");
            }else{
                for(int i = 0; i < numberCards; i++){
                    hand.add(playingDeck.get(0));
